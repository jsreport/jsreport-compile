...
function collectConfig (input) {
  ...
  var config = {
    ...
    exposeIncludes: [{
      expose: 'nexeres',
      path: './' + path.relative(process.cwd(), path.join(nodeTempDir, 'lib/nexeres.js'))
    }],
    excludes: [
...
function compile (label, config, options) {
  ...
  config.exposeIncludes.forEach((e) => compileConfig.browserifyRequires.push({ file: e.path, expose: e.expose }))
...
function compileJsreport (config, options) {
  return compile('jsreport', config, Object.assign({}, options, {
...
function compileExe (id, version, options) {
  ...
  var config = {
    ...
    includes: {
      // include the jsreport bundle in exe
      jsreportRuntime: './jsreportRuntime.js',
      daemonInstance: './node_modules/jsreport-cli/lib/daemonInstance.js'
    },
    exposeIncludes: [{
      expose: 'nexeres-ligth',
      path: './' + path.relative(process.cwd(), path.join(nodeTempDir, 'lib/nexeres.js'))
    }],
    excludes: [],
    modules: []
  ...
  return validateResources(config).then(function () {
    return validateIncludes(config)
  }).then(function () {
    writeStartup(config, options)

    return compile('executable', config, options)
  })
}
...
module.exports = function (options) {
  ...
  return prepareJsreport(id, options).then(function (config) {
    jsreportConfig = config

    // applying patches for both bundles
    patches()

    // creating jsreport bundle
    return compileJsreport(jsreportConfig, options)
  }).then(function () {
    // creating the final exe
    return compileExe(id, jsreportConfig.version, options)
  }).catch(function (e) {
    console.error(e)
    throw e
  }).finally(function () {
