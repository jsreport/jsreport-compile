...
function collectConfig (input) {
  ...
  var resourcesId = './' + path.relative(process.cwd(), path.join(nodeTempDir, 'lib/nexeres.js'))
...
function compileExe (id, version, options) {
  ...
  var config = {
    ...
    resources: {
      ...
      // including the jsreport bundle in exe as a temp file
      'jsreportRuntime.js': { path: path.relative(process.cwd(), './jsreportRuntime.js'), temp: true }
    },
    includes: {
      // include resources
      'nexeres-ligth': resourcesId,
