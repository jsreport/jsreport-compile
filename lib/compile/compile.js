...
function writeStartup (label, config, options) {
  ...
  var extensions = 'function requireExtensions() { return [' + (
    config.extensions.length === 0 ? '' : config.extensions.map(function (e) {
      const extWithCommands = config.extensionsCommands ? config.extensionsCommands.find((ex) => ex.name === e.name) : null
      let cliModulePath
      ...
      }'), { source: '${e.source}', version: '${e.version}', cliModule: ${cliModulePath == null ? 'null' : `require('./${
