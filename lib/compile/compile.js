...
function collectConfig (options) {
  ...
  return reporter.init().then(function () {
    ...
    if (reporter.cli) {
      return reporter.cli.findCommandsInExtensions().then(function (extensionsCommands) {
        config.extensionsCommands = extensionsCommands
      })
    } else {
    ...
    reporter.options.templatingEngines.nativeModules.forEach(function (e) {
      config.includes[e.module] = './' + path.relative(process.cwd(), e.module).replace(/\\/g, '/')
    })

    // the same for modules
    reporter.options.templatingEngines.modules.forEach(function (m) {
...
function writeStartup (label, config, options) {
  ...
  var cliExtensionsCommands = 'function requireCliExtensionsCommands() { return [' + (
    config.extensionsCommands.length === 0 ? '' : config.extensionsCommands.map(function (e) {
...
function compileExe (id, jsreportInfo, options) {
  ...
  var config = {
    ...
    nodeTempDir: nodeTempDir,
    extensions: [],
    extensionsCommands: jsreportInfo.config.extensionsCommands,
