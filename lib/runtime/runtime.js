...
module.exports = function (options) {
  const execution = new Execution('nexeres-ligth', options.resources, options.includes, options.version, options.shortid)
  const getJsreport = prepareJsreport(execution.resolve('jsreportRuntime'))
  ...
  if (process.argv.length === 5 && process.argv[4] === '--node-process') {
    ...
    execution.ensureExeResources(['WinRun.exe', 'nssm.exe', 'nssm64.exe']).then(function () {
