...
module.exports = function (options) {
  ...
  jsreport.afterConfigLoaded(function (reporter) {
    ...
    if (reporter.options.logger.providerName === 'winston') {
      ...
      reporter.options.logger.providerName == null &&
      !optionsContainsTransports(omit(reporter.options.logger, ['silent', 'logDirectory', 'providerName']))
    ) {
      transportSettings = {
        console: consoleTransport
      }
    ...
    if (
      ...
      transportSettings.main = assign({}, mainTransport, reporter.options.logger.main)
    }
